...
export function useAsyncData<T>(path: string, defaulParams?:any): [IAsyncData<T>, (params?:any)=> void]{
    ...
    const getData = React.useCallback((params?:any)=>{
        if (!!path) {
            ...
            axios.get<T>(`${BASE_URL}${path}`, {params: params ?? defaulParams}).then(({data}) => {
